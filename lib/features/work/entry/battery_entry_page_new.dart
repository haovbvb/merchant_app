...
class BatteryEntryPageNew extends StatefulWidget {
  const BatteryEntryPageNew({super.key, this.selectedType});

  final BatteryType? selectedType;

  @override
  State<BatteryEntryPageNew> createState() => _BatteryEntryPageNewState();
}

class _BatteryEntryPageNewState extends State<BatteryEntryPageNew> {
  ...
  Widget _buildModelHeader(BatteryType type) {
    final l10n = context.l10n;
    ...
    await Navigator.of(context).push<void>(
      MaterialPageRoute(
        builder: (_) => QrScanPage(
          ...
          returnRaw: true,
          continuousScan: true,
          onContinuousScan: _handleContinuousBatteryScan,
        ),
      ),
    );
  }

  String _handleContinuousBatteryScan(String rawValue) {
  ...
  Future<_BatteryItem?> _showManualEntrySheet({_BatteryItem? initial}) async {
    final l10n = context.l10n;
    ...
    return showModalBottomSheet<_BatteryItem>(
      context: context,
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              // Battery SN
              _buildInputField(
                label: l10n.entryBatterySnLabel,
                hint: l10n.entryBatterySnHint,
                ...
                onScan: () async {
                  final result = await Navigator.of(context).push<String>(
                    MaterialPageRoute(
                      builder: (_) =>
                          const QrScanPage(parseDeviceSn: true, deviceType: 1),
                    ),
                  );
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (goToShip == true) {
        ...
        await Navigator.of(context).pushReplacement(
          MaterialPageRoute(
            builder: (_) => EntrySuccessPage(title: l10n.batteryEntryTitle),
          ),
        );
      }
    } finally {
