class ScanUtils {
  ...
  static bool ensureNoChinese(String value) {
    ...
    return !_chinesePattern.hasMatch(value);
  }
  ...
  static QrCodeBattery parseBatteryQr(String qr, int clickType) {
    ...
    if (qr.contains(',') && _isContainsKeyValue(qr)) {
      final map = _splitKeyValue(qr, lowerKeys: true);
      return QrCodeBattery(
        sn: map['sn'],
        imei: map['imei'],
        iccid: map['iccid'],
      );
  ...
  static QrCodeVehicle parseVehicleQr(String qr, int clickType) {
    ...
    if (qr.contains(',') && _isContainsKeyValue(qr)) {
      ...
      return QrCodeVehicle(sn: sn, vin: vin, vcu: map['VCU']);
    }
  ...
  static bool _isContainsKeyValue(String input) {
    ...
    return tokens.every((token) => token.contains(':') || token.contains('='));
  }
}
