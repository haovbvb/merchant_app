...
//            para[i] = (byte) value;
        ...
        return calulateCRCbySoft(datas);
    }


    // CRC 效验算法
    public static String calulateCRCbySoft(int[] pucData) {
        int ucTemp = 0;
        int wValue = 0;
        int[] pOutData = new int[2];
        int crc_tbl[] = {
...
//            Logger.d("i:" + i);
            ...
            int temp = ucTemp ^ ((pucData[i] >> 4));
            wValue ^= crc_tbl[temp] & 0xFFFF;
            // 再计算低四位余式
            ucTemp = (((wValue >> 8)) >> 4);
            wValue = (wValue << 4) & 0xFFFF;
            temp = ucTemp ^ ((pucData[i]) & 0x0f);
            wValue ^= crc_tbl[temp] & 0xFFFF;
        }
        pOutData[1] = wValue & 0xFF;
        pOutData[0] = (wValue >> 8) & 0xFF;
        String crc0 = Integer.toHexString(pOutData[0]);
        String crc1 = Integer.toHexString(pOutData[1]);
        if (crc0.length() != 2) {
            crc0 = '0' + crc0;
        }
        if (crc1.length() != 2) {
