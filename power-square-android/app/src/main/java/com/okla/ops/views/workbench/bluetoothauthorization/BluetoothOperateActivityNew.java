...
import com.okla.ops.databinding.ActivityBluetoothOperateNewBinding;
 ...
 * 蓝牙操作 指令
 */
public class BluetoothOperateActivityNew extends BaseNormalVActivity<BluetoothAuthorizationViewModel, ActivityBluetoothOperateNewBinding> implements View.OnClickListener {
    ...
    public static void startBluetoothOperateActivityNew(Context context, BleDevice bleDevice, BluetoothGatt gatt, int status) {
        Intent intent = new Intent(context, BluetoothOperateActivityNew.class);
...
//        intent.putExtra(BLUETOOTH_GATT, gatt);
    ...
    protected int getLayoutId() {
        return R.layout.activity_bluetooth_operate_new;
    }
    ...
    protected void initViews(Bundle savedInstanceState) {
        ...
        mBinding.tvClearAuthorize.setOnClickListener(v -> {
            //清空授权
    ...
    private QuickPopup mOpenCabinetTipDialog;

    /**
     * 操作开柜授权提示
     */
    private void showOpenCabinetTipDialog() {
        mOpenCabinetTipDialog = QuickPopupBuilder.with(mContext).contentView(R.layout.dialog_warning)
                .config(new QuickPopupConfig()
                        .gravity(Gravity.CENTER)
                        .outSideTouchable(false)
                        .outSideDismiss(false)
                        .backpressEnable(false)
                        .withClick(R.id.btn_cancel, v -> mOpenCabinetTipDialog.dismiss())
                        .withClick(R.id.btn_ok, v -> {
                            ...
                            mOpenCabinetTipDialog.dismiss();
                        })).build();
        mOpenCabinetTipDialog.showPopupWindow();
        mOpenCabinetTipDialog.getContentView().post(()->{
            TextView tvContent = mOpenCabinetTipDialog.findViewById(R.id.tvContent);
            String strTip = getString(R.string.open_cabinet_authorization_tip);
    ...
    private void showTipDialog(int type) {
        mTipDialog = QuickPopupBuilder.with(mContext).contentView(R.layout.dialog_warning)
                .config(new QuickPopupConfig()
                        .gravity(Gravity.CENTER)
                        .outSideTouchable(false)
                        .outSideDismiss(false)
    ...
    private void addObserver() {
        getViewModel().mBluetoothEnOrDecrypt.observeForever(result -> {
            ...
            if (isEncrypt) {
                ...
                Logger.d("解密成功-------");
                BluetoothConstants.commandBack = "";
                if ("0031".equals(mSendSignal)) {
                    //获取钥匙id
                    Logger.d("获取钥匙id-------");
                    getKeyID(result.data);
                    return;
                }
                if ("000c".equals(mSendSignal)) {
                ...
                } else if ("0087".equals(mSendSignal)) {
                    getLoading().onFinish();
                ...
                } else if ("000D".equals(mSendSignal)) {
                    Logger.d("授权入口:");
                    String backKeyId = result.data.substring(0, 8);
                    if (backKeyId.equals(keyId)) {
