package com.okla.ops.views.workbench.entry

import android.Manifest
import android.content.Context
import android.content.Intent
import android.graphics.Color
import android.os.Bundle
import android.text.Spannable
import android.text.SpannableString
import android.text.style.ForegroundColorSpan
import android.view.View
import androidx.lifecycle.ViewModelProvider
import com.base.common.base.mvvm.BaseNormalVActivity
import com.base.common.dialog.CustomDialog
import com.base.common.permission.PermissionListenerImpl
import com.base.common.permission.PermissionManager
import com.base.common.qrcode.QrCodeUtils
import com.base.common.utils.ToastUtils
import com.bumptech.glide.Glide
import com.google.zxing.integration.android.IntentIntegrator
import com.okla.ops.R
import com.okla.ops.beans.DeviceTypeObject
import com.okla.ops.beans.ScanDeviceBean
import com.okla.ops.databinding.ActivityBatteryEntryBinding
import com.okla.ops.dialog.DialogBatteryInput
import com.okla.ops.utils.ScanUtils
import com.okla.ops.views.workbench.QRCodeActivity
import com.okla.ops.views.workbench.QRCodeListActivity
import com.okla.ops.views.workbench.warehouse.devicetransport.DeviceTransportCreateActivity
import com.luck.picture.lib.utils.SdkVersionUtils
import com.okla.ops.beans.StationNew
import com.okla.ops.beans.StationType

class StationEntryActivity :
    BaseNormalVActivity<DeviceShipViewModel, ActivityBatteryEntryBinding>() {
    private val snList = ArrayList<String>()
    private var station: StationType? = null

    companion object {
        fun startStationEntryActivity(context: Context, stationType: StationType) {
            val intent = Intent(context, StationEntryActivity::class.java)
            intent.putExtra("stationType", stationType)
            context.startActivity(intent)
        }

    }

    override fun onCreateViewModel(): DeviceShipViewModel {
        return ViewModelProvider(this)[DeviceShipViewModel::class.java]
    }

    override fun getLayoutId(): Int {
        return R.layout.activity_battery_entry
    }

    override fun title(): Int {
        return R.string.title_station_entry
    }

    override fun getIntentExtras(intent: Intent?) {
        station = intent?.getParcelableExtra("stationType")
    }

    private lateinit var permissionManager: PermissionManager
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        permissionManager = PermissionManager.getInstance(
            this.activityResultRegistry, this.javaClass.name, this
        )
        lifecycle.addObserver(permissionManager)
        initObserver()
    }

    override fun initViews(savedInstanceState: Bundle?) {
        super.initViews(savedInstanceState)
        initClick()
        initData()
    }

    private fun initObserver() {
        getViewModel().newStationListLiveData.observe(this) {
            val newStationCount = getViewModel().getNewStationCount();
            mBinding.btnConfirm.isEnabled = newStationCount > 0
            if (newStationCount > 0) {
                mBinding.layoutScrollview.visibility = View.VISIBLE
                mBinding.layoutEmpty.visibility = View.GONE
            } else {
                mBinding.layoutScrollview.visibility = View.GONE
                mBinding.layoutEmpty.visibility = View.VISIBLE
            }
            mBinding.tvTotal.text =
                getString(R.string.text_station_total, newStationCount)
        }
        var gotoShipMsg = getString(R.string.dialog_go_to_ship_stations)
        getViewModel().registerStationLiveData.observe(this) {
            CustomDialog.Builder(this)
                .setTitle(getString(R.string.dialog_submit_completed))
                .setMessage(gotoShipMsg)
                .setNegativeButton(com.base.common.R.string.cc_str_close) { dialog, _ ->
                    dialog.dismiss()
                    ShipSucActivity.startActivity(
                        mContext,
                        "",
                        DeviceTypeObject.TYPE_STATION
                    )
                    finish()
                }
                .setPositiveButton(R.string.title_ship) { dialog, _ ->
                    dialog.dismiss()
                    getViewModel().getNewStationList().forEach { newBattery ->
                        snList.add(newBattery.sn)
                    }
                    DeviceTransportCreateActivity.getIntents(
                        this,
                        DeviceTypeObject.TYPE_STATION,
                        snList
                    )
                    finish()
                }.create().show()
        }
    }

    private fun initClick() {
        mBinding.btnManualEntry.setOnClickListener(this)
        mBinding.btnScanEntry.setOnClickListener(this)
        mBinding.btnConfirm.setOnClickListener(this)
    }

    private fun initData() {
        mBinding.tvBatteryDimension.text = mContext.getString(R.string.text_dimension)
        mBinding.tvBatteryWeight.text = mContext.getString(R.string.text_net_weight)
        station?.let {
            Glide.with(this).load(it.img).into(mBinding.ivBattery)
            mBinding.tvBatteryModel.text = "${it.model} (${it.modelName})"
            mBinding.tvBatteryDetail.text = it.remark
            mBinding.tvDimension.text = it.dimension ?: "-"
            mBinding.tvWeight.text = it.weight ?: "-"
        }
        mBinding.tvTotal.text =
            getString(R.string.text_station_total, getViewModel().getNewStationCount())

    }

    override fun onNoDoubleClick(v: View) {
        super.onNoDoubleClick(v)
        when (v.id) {
            R.id.btnManualEntry -> {
                showDialogStationInput(0, null, getString(R.string.btn_manual_entry))
            }

            R.id.btnScanEntry -> {
                mScanType = 1
                askPermission()
            }

            R.id.btnConfirm -> {
                val newBatteryCount = getViewModel().getNewStationCount();
                if (newBatteryCount <= 0) {
                    ToastUtils.showShort(getString(R.string.tips_station_not_empty))
                    return
                }
                var stationStr = getString(R.string.text_stations)
                val string = getString(
                    R.string.dialog_battery_content,
                    "$newBatteryCount $stationStr"
                )
                val start = string.indexOf("$newBatteryCount")
                val end = string.indexOf(stationStr) + stationStr.length
                val spannableString = SpannableString(string)
                val color = Color.parseColor("#FF56B327")
                spannableString.setSpan(
                    ForegroundColorSpan(color),
                    start,
                    end,
                    Spannable.SPAN_EXCLUSIVE_EXCLUSIVE
                )
                CustomDialog.Builder(this)
                    .setTitle(getString(R.string.dialog_confirm_submit))
                    .setSpannableMessage(spannableString)
                    .setNegativeButton { dialog, _ -> dialog.dismiss() }
                    .setPositiveButton { dialog, _ ->
                        dialog.dismiss()
                        getViewModel().registerStation(station?.model ?: "")
                    }.create().show()
            }
        }
    }

    private fun addStationItem(
        index: Int,
        batterSn: String,
        imei: String,
        iccid: String,
        lockDevId: String
    ) {
        val result = getViewModel().addNewStation(StationNew(batterSn, imei, iccid, lockDevId))
        if (result) {
            // 添加后重建整个列表
            rebuildStationListFromViewModel()
        }
    }

    private fun updateStationItem(
        index: Int,
        batterSn: String,
        imei: String,
        iccid: String,
        lockDevId: String
    ) {
        val updateResult =
            getViewModel().updateNewStationInfo(index, batterSn, imei, iccid, lockDevId)
        if (!updateResult) {
//            ToastUtils.showShort(getString(R.string.tips_station_sn_duplicate))
            return
        }
        // 强制重建整个列表以确保UI同步
        rebuildStationListFromViewModel()
    }

    private fun rebuildStationListFromViewModel() {
        mBinding.llBattery.removeAllViews()
        val list = getViewModel().newStationListLiveData.value ?: return

        list.forEachIndexed { i, station ->
            val item = BatteryInfoItem(this)
            item.setData(
                station.sn,
                station.imei,
                station.iccid,
                station.lockDevId,
                DeviceTypeObject.TYPE_STATION
            )

            item.setOnBatteryInfoListener(object : BatteryInfoItem.OnBatteryInfoListener {
                override fun onEdit(v: BatteryInfoItem) {
                    // 通过SN来查找在ViewModel中的实际索引
                    val actualIndex = list.indexOfFirst { it.sn == v.getSn() }
                    if (actualIndex != -1) {
                        showDialogStationInput(
                            actualIndex,
                            v,
                            mContext.getString(R.string.title_edit_info)
                        )
                    }
                }

                override fun onDelete(v: BatteryInfoItem) {
                    getViewModel().removeNewStation(v.getSn())
                    // 删除后重建列表
                    rebuildStationListFromViewModel()
                }
            })
            mBinding.llBattery.addView(item)
        }
    }


    private fun askPermission() {
        val permissions = mutableListOf(
            Manifest.permission.READ_EXTERNAL_STORAGE,
            Manifest.permission.CAMERA,
        )
        if (SdkVersionUtils.isTIRAMISU()) {
            permissions.remove(Manifest.permission.READ_EXTERNAL_STORAGE)
        }
        permissionManager.checkPermissions(object : PermissionListenerImpl() {
            override fun passPermission() {
                super.passPermission()
                if (mScanType == 1) {
                    val list = getViewModel().newStationListLiveData.value ?: mutableListOf()
                    val scanBeanList: ArrayList<ScanDeviceBean> = arrayListOf()
                    list.forEach {
                        scanBeanList.add(ScanDeviceBean(it.sn, it.lockDevId, ""))
                    }
                    startActivityForResult(
                        QRCodeListActivity.getIntents(
                            activity,
                            false,
                            scanBeanList,
                            false, true, DeviceTypeObject.TYPE_STATION
                        ),
                        QrCodeUtils.REQUEST_CODE_SCAN_SN_LIST
                    );
//                    startActivityForResult(
//                        ScanSnQrCodeListActivity.getIntents(activity, false, snList),
//                        QrCodeUtils.REQUEST_CODE_SCAN_SN_LIST
//                    )
                } else {
//                    startActivityForResult(Intent(this@BatteryEntryActivity, QRCodeActivity::class.java),QrCodeUtils.REQUEST_CODE_DEFINE)
//                    QrCodeUtils.startScanQrCode(this@BatteryEntryActivity)
                    IntentIntegrator(this@StationEntryActivity)
                        .setOrientationLocked(false)
                        .addExtra(QRCodeActivity.QR_TYPE_TARGET, QRCodeActivity.NORMAL)
                        .setCaptureActivity(QRCodeActivity::class.java)
                        .initiateScan() //  初始化扫描
                }
            }

            override fun showRequestPermissionRationale() {
                super.showRequestPermissionRationale()
                PermissionManager.askForPermission(this@StationEntryActivity, "")
            }
        }, *permissions.toTypedArray())
    }


    private var dialogStationInput: DialogBatteryInput? = null
    private fun showDialogStationInput(
        index: Int,
        stationInfoItem: BatteryInfoItem?,
        title: String
    ) {
        if (dialogStationInput == null) {
            dialogStationInput =
                DialogBatteryInput(
                    this,
                    title,
                    DeviceTypeObject.TYPE_STATION ?: 0
                )
        }
        dialogStationInput?.setOnInputListener(object :
            DialogBatteryInput.OnInputListener {
            override fun onConfirm(
                index: Int,
                batterSn: String?,
                imei: String?,
                iccid: String?,
                locDevId: String?
            ) {
                if (stationInfoItem == null) {
                    addStationItem(index, batterSn ?: "", "", "", locDevId ?: "")
                } else {
                    updateStationItem(index, batterSn ?: "", "", "", locDevId ?: "")
                }
            }

            override fun onScan(scanType: Int) {
                mScanType = scanType
                askPermission()
            }

        })
        dialogStationInput?.updateTitle(title)
        dialogStationInput?.setIndex(index)
        if (stationInfoItem == null) {
            dialogStationInput?.updateBatterySn("")
            dialogStationInput?.updateIMEI("")
            dialogStationInput?.updateICCID("")
            dialogStationInput?.updateLockDevId("")
        } else {
            dialogStationInput?.updateBatterySn(stationInfoItem.getSn())
            dialogStationInput?.updateIMEI("")
            dialogStationInput?.updateICCID("")
            dialogStationInput?.updateLockDevId(stationInfoItem.getLockDevId())
        }
        dialogStationInput?.show()
    }

    private var mScanType: Int = 0
    var mScanResult: String = ""

    /**
     * Event for receiving the activity result.
     *
     * @param requestCode Request code.
     * @param resultCode Result code.
     * @param data        Result.
     */
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        super.onActivityResult(requestCode, resultCode, data)
        if (data == null) {
            return
        }
        if (requestCode == QrCodeUtils.REQUEST_CODE_SCAN_SN_LIST) {
            if (mScanType == 1) {//sn list
                val list =
                    data?.getParcelableArrayListExtra<ScanDeviceBean>(QRCodeListActivity.SN_LIST)
                list?.let {
                    it.forEachIndexed { index, scanDeviceBean ->
                        addStationItem(index, scanDeviceBean.sn?:"", "", "", scanDeviceBean.vin?:"")
                    }
                }
            }
        } else if (requestCode == IntentIntegrator.REQUEST_CODE) {
            val intentResult = IntentIntegrator.parseActivityResult(requestCode, resultCode, data)
            mScanResult = intentResult?.contents.toString()
            if (mScanResult.matches(ScanUtils.chinesePattern)) {
                ToastUtils.showShort(mContext.getString(R.string.tip_no_chinese))
                return
            }
//            if (mScanType == 2) {//input sn
//                val deviceSn = ScanUtils.getDeviceSn(mContext, mScanResult)
//                dialogBatteryInput?.updateBatterySn(deviceSn)
//            }
//            if (mScanType == 3) {//imei
//                if (mScanResult != null) {
//                    dialogBatteryInput?.updateIMEI(mScanResult)
//                }
//            }
//            if (mScanType == 4) {//iccid
//                if (mScanResult != null) {
//                    dialogBatteryInput?.updateICCID(mScanResult)
//                }
//            }
            if (mScanType == 2 || mScanType == 5) {
                val qrcodeStation = ScanUtils.parseStationQr(mContext, mScanResult)
                dialogStationInput?.updateBatterySn(qrcodeStation?.sn)
                dialogStationInput?.updateLockDevId(qrcodeStation?.lockDevId)
            }
        }
    }
}