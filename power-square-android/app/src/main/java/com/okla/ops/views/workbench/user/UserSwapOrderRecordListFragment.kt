...
class UserSwapOrderRecordListFragment :
    BaseNormalVFragment<UserViewModel, FragmentRefreshListBinding>() {
    private var pageIndex = 1
    private var pageSize = 10

    companion object {
        fun getInstance(cardNum: String): UserSwapOrderRecordListFragment {
            val fragment = UserSwapOrderRecordListFragment()
            val bundle = Bundle()
    ...
    private fun addObserver() {
        ...
        getViewModel().userSwapOrderListLiveData.observe(this) {
            updateList(it)
        }
    }

    @SuppressLint("NotifyDataSetChanged")
    private fun updateList(items: List<OtherOrder>?) {
    ...
    private lateinit var mAdapter: SingleDataBindingNoPUseAdapter<OtherOrder>
    private fun initAdapter() {
        mAdapter = object :
            SingleDataBindingNoPUseAdapter<OtherOrder>(R.layout.item_userdetail_swaporder) {
            override fun convert(
                helper: BaseViewHolder, item: OtherOrder
            ) {
                ...
                helper.setText(R.id.tv_vehicle, item.carType)
                helper.setText(R.id.tv_battery, item.batteryType.plus(" • ").plus(item.batteryNum))
                helper.setText(
                    R.id.tv_price,
                ...
                helper.setText(
                    R.id.tv_swaptime,
                    item.times.toString().plus(mContext.getString(R.string.str_times))
                )
                helper.setText(
                    R.id.tv_remainday,
                ...
                helper.setText(
                    R.id.tv_remaintime,
                    item.remainTime.toString().plus(mContext.getString(R.string.str_times))
                )
                helper.setText(
                    R.id.tv_expir_date,
                ...
                val tvPayType = helper.getView<TextView>(R.id.tv_paytype)
                var str1 = ""
                var str2 = ""
                ...
                when (item.status) {
                    ...
                    0 -> {
                        ...
                        helper.setText(
                            R.id.tv_remaintime,
                            "-".plus(mContext.getString(R.string.str_times))
                        )
                    }
    ...
    private fun refreshData() {
        getViewModel().getUserOrderList(2, mCardNum, pageIndex, pageSize)
    }
