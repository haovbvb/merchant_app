...
public class SlideButton extends AppCompatCheckBox {
     ...
     * 开关选中状态,即打开状态时的背景颜色
     */
    private int mBackgroundColorChecked = 0xFF2D85FF;
    /**
     * 开关指示器按钮的颜色
     */
    ...
    public SlideButton(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            ...
            int drawableResId = ta.getResourceId(R.styleable.SlideButton_drawableSelector, -1);
            ta.recycle();

            if (drawableResId != -1) {
                stateListDrawable = (StateListDrawable) ContextCompat.getDrawable(context, drawableResId);
            }
        }
...
//            x = radius + mPaint.getStrokeWidth() + mPaint.getStrokeWidth();
        ...
        if (stateListDrawable != null) {
            ...
            // 设置绘制区域（填充整个 View）
            if (isChecked()) {
                stateListDrawable.setBounds(getWidth() / 2 - (getWidth() / 16), 0, getWidth() - (getWidth() / 16), getHeight());
            } else {
                stateListDrawable.setBounds(getWidth() / 16, 0, getWidth() / 2 + (getWidth() / 16), getHeight());
            }
            // 绘制 selector 背景
    ...
    public void setStateListDrawable(int drawable) {
        stateListDrawable = (StateListDrawable) ContextCompat.getDrawable(getContext(), drawable);
    }
